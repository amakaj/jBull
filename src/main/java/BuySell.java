...
public class BuySell {
	...
	private static HashMap<String, Integer> stockData;
	
	public static void updateBalances(User currentUser) throws IOException {		
	...
	public BuySell(User currentUser) {
		...
		stockData = currentUser.getStockData();
				...
				try {
					if ((numOfSharesField.getText() != null && !numOfSharesField.getText().equals("") && Integer.parseInt(numOfSharesField.getText()) > 0)
						...
						try {
							...
							priceLabel.setText("Price: $" + priceOfFieldShare);
						} catch (IOException ex) {
							ex.printStackTrace();
						}
					} else {
